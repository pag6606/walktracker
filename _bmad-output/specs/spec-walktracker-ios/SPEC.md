...
- **Licencias:** en **código**, solo dependencias Apache-2.0 / MIT; copyleft fuerte (GPL/AGPL) es bloqueante. En **fuentes de datos** se admite CC-BY 4.0 con atribución visible — Open-Meteo lo es, y su atribución va **en dos sitios y los dos son obligatorios**: Ajustes → Acerca de, y junto al dato en la tarjeta de clima.
  > ✅ **CERRADO el 2026-09-21 (`B-9`).** Desde el 2026-09-20 esta línea llevaba un ⚠️ que decía "abierto — no está cumplido": la atribución vivía solo en `WeatherCard.swift`, **dentro de `if let weather`**, invisible sin clima y fuera de una sesión, y la 2.3 creó Ajustes y la excluyó con un comentario, que no es un destino. Ya existe la sección "Acerca de" (`WalkTracker/UI/Settings/SettingsView.swift` + `AboutSection.swift`), y la de la tarjeta **se queda**, porque Open-Meteo pide el enlace *junto al dato*. **La cláusula está transcrita de su fuente en `NOTICE`** (raíz del repo), con fecha, huella del fichero descargado y un mapa de qué línea del producto cumple qué requisito — incluido el que **no** se cumple: la app nombra la licencia pero no enlaza a su texto, registrado con destino en `deferred-work.md`. [`ARCHITECTURE-SPINE.md` AD-24; `NOTICE`; `spec-b9-degradacion-y-atribucion.md`]
...
**OQ-5 RESUELTA (Paul, 2026-09-12):** el iPhone 14 de validación **se congela en iOS 26** y no se actualiza a iOS 27 hasta terminar el desarrollo. El toolchain se congela con él: **Xcode 26.3 (build 17C529) / Swift 6.2.4**. Consecuencias aceptadas, registradas aquí porque vinculan:

> ⚠️ **ENMENDADO el 2026-09-20.** Esta línea decía *"Xcode 26.6 / Swift 6.3.3"*, números que nunca se verificaron contra la máquina. La de Paul responde **Swift 6.2.4** y **Xcode 26.3 (17C529)**, que es lo que la tabla de Stack del spine ya decía y lo que su registro de riesgos afirma ahora. La decisión de Paul —congelar el dispositivo y el toolchain— no cambia; cambian los números. [verificado con `swift --version` y `xcodebuild -version`; `ARCHITECTURE-SPINE.md`, tabla de Stack y registro de riesgos]
