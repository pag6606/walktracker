...
- **Licencias:** en **código**, solo dependencias Apache-2.0 / MIT; copyleft fuerte (GPL/AGPL) es bloqueante. En **fuentes de datos** se admite CC-BY 4.0 con atribución visible — Open-Meteo lo es, y su atribución **debe ir** en Ajustes → Acerca de.
  > ⚠️ **ABIERTO al 2026-09-20 — no está cumplido, y esta línea lo daba por hecho.** Decía que la atribución *"va"* en Ajustes → Acerca de, en presente, como si describiera el producto. **No es así:** acabó en `WeatherCard.swift`, **dentro de `if let weather`**, así que no se ve sin clima ni fuera de una sesión; la 2.3 creó la pestaña Ajustes y la excluyó con un comentario, que no es un destino. Es una **obligación de licencia incumplida**. **Dueño: el action item `B-9`** de la retrospectiva del Epic 2 — aquí el documento deja de afirmar que está resuelto, y nada más. [`WalkTracker/UI/Session/WeatherCard.swift:51`, `WalkTracker/UI/Settings/SettingsView.swift:7`; `ARCHITECTURE-SPINE.md` AD-24]
