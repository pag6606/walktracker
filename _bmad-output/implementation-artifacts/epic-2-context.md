...
- Open-Meteo exige **atribución visible** en Ajustes → Acerca de. Ningún criterio de aceptación la pide: hay que asignarla a una historia (2.1) o registrarla en `deferred-work.md` con destino explícito (**NFR-7**, **AD-24**).
  > **RESUELTO el 2026-09-21 (B-9).** No pasó ninguna de las dos cosas: acabó dentro de `if let weather` en la tarjeta de clima, sin destino registrado (D6 de la retro del Epic 2). Hoy está **en los dos sitios** —"Acerca de" en Ajustes y junto al dato en la tarjeta—, y la obligación tiene dueño escrito en `NOTICE`, en AD-24 y en la nota de `epics.md` bajo la 2.3.
...
- **Permisos y degradación (AD-11).** El adapter posee el estado de su permiso y lo expone por el puerto como `status`; toda petición va precedida de pre-pantalla explicativa. La respuesta a "falta esta capacidad" **es la tabla de AD-11, y la cumple cada frontera en su sitio**: "ubicación denegada" y "red no disponible" significan sesión sin clima, nunca bloqueo. La retro del Epic 1 (**S6**) dejó **dos huecos concretos** que este epic tenía que reconciliar:
  - ~~**`DegradationPolicy` no existe todavía** en el árbol Swift: hoy cada degradación se decide en su sitio.~~ → **RESUELTO el 2026-09-21 (B-9), y en contra de lo que este bullet daba por supuesto.** El encargo no se hizo durante el epic (D6 de la retro del Epic 2). Al recogerlo, la decisión de Paul fue **enmendar AD-11 y no escribir el tipo**: tendría dos llamantes, no eliminaría ningún `switch` y no capturaría lo que varía entre filas. "Cada degradación se decide en su sitio" deja de ser una desviación y pasa a ser **la regla**. Ver la enmienda de AD-11 en `ARCHITECTURE-SPINE.md`, que además dice qué tres filas de las seis no tienen ni una línea.
  - **`PermissionStatus.unavailable` hoy bloquea igual** que el permiso denegado de Motion. La ubicación **no** debe heredar ese bloqueo. → **Cumplido por la 2.1:** `SessionStore.beginWeatherForNewSession()` trata `.unavailable` como los demás estados no concedidos —sesión sin clima, sin diálogo—, y el bloqueo de `.unavailable` se queda donde debía, en el flujo de Motion.
